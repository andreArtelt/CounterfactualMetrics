...
def plot_distmats_boxplot(data, title=None, show=True):
    plt.figure()
...
def plot_distmat(mat, title=None, show=True):
    plt.matshow(mat)
    plt.title(title)
    if show is True:
        plt.show()
...
def plot_2d_decisionboundary(model, X, y, X_old=None, y_old=None, alpha_old=0.1, grid_resolution=0.1, color_data={'colors': ['r', 'b', 'g', 'y'], 'c_light': ['#FFAAAA', '#AAAAFF', '#AAFFAA', '#FFFFAA'],
    ...
    if X_old is not None and y_old is not None:
        plt.scatter(X_old[:, 0], X_old[:, 1], alpha=alpha_old, c=list(map(lambda i: color_data['colors'][i], y_old)), cmap=cmap_bold, edgecolor='k', s=20)
